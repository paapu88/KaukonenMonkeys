...
def init_db(remove_file=None):
    ...
    adminf = Friend(name='admin', monkey = guest)
    guestf = Friend(name='guest', monkey = veeti)
    veetif = Friend(name='veeti', monkey = kerttu)
    kerttuf = Friend(name='kerttu', monkey = kukka)
    kukkaf = Friend(name='kukka', monkey = admin)
    db.session.add(adminf)
    db.session.add(guestf)
    db.session.add(veetif)
    db.session.add(kerttuf)
    db.session.add(kukkaf)
    db.session.commit()

    adminf = Friend(name='admin', monkey = veeti)
    guestf = Friend(name='guest', monkey = kerttu)
    veetif = Friend(name='veeti', monkey = kukka)
    db.session.add(adminf)
    db.session.add(guestf)
    db.session.add(veetif)
    db.session.commit()
